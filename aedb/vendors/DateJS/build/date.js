 ...
 */(function () {
	...
	var buildInfo = {
		...
		days: function () {
			return __(["Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sabado"]);
		},
		dayAbbr: function () {
			return __(["Dom", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]);
		},
		dayShortNames: function () {
			return __(["Dom", "Mo", "Tu", "We", "Th", "Fr", "Sa"]);
		},
...
(function () {
	 ...
	 * @param {String}   The name of the day (eg. "Segunda, "Mon", "Terça", "tue", "We", "we").
...
(function () {
	...
	/**
	 * Get the week number. Week one (1) is the week which contains the first Quinta of the year. Segunda is considered the first day of the week.
	...
	$P.getWeek = function (utc) {
		...
		// ISO week date weeks start on Segunda  
		// so correct the day number  
		var dayNr = (self.getDay() + 6) % 7;
		// ISO 8601 states that week 1 is the week  
		// with the first Quinta of that year.  
		// Set the target date to the Quinta in the target week  
		target.setDate(target.getDate() - dayNr + 3);
		// Store the millisecond value of the target date  
		var firstQuinta = target.valueOf();
		// Set the target to the first Quinta of the year  
		// First set the target to january first  
		target.setMonth(0, 1);
		// Not a Quinta? Correct the date to the next Quinta  
		if (target.getDay() !== 4) {
			target.setMonth(0, 1 + ((4 - target.getDay()) + 7) % 7);
		}
		// The weeknumber is the number of weeks between the   
		// first Quinta of the year and the Quinta in the target week  
		return 1 + Math.ceil((firstQuinta - target) / 604800000); // 604800000 = 7 * 24 * 3600 * 1000  
	};
	
	/**
	 * Get the ISO 8601 week number. Week one ("01") is the week which contains the first Quinta of the year. Segunda is considered the first day of the week.
	...
	/**
	 * Moves the date to Segunda of the week set. Week one (1) is the week which contains the first Quinta of the year.
	 ...
	 * dddd   The full day name. Date.CultureInfo.dayNames.                                              "Segunda" to "Domingo"
	 * 
...
(function () {
	...
	/** 
	 * Moves the date to the next instance of a date as specified by the subsequent date element function (eg. .day(), .month()), month name function (eg. .january(), .jan()) or day name function (eg. .Sexta(), fri()).
	 * Example
	<pre><code>
	Date.today().next().Sexta();
	Date.today().next().fri();
	...
	/** 
	 * Creates a new Date (Date.today()) and moves the date to the next instance of the date as specified by the subsequent date element function (eg. .day(), .month()), month name function (eg. .january(), .jan()) or day name function (eg. .Sexta(), fri()).
	 * Example
	<pre><code>
	Date.next().Sexta();
	Date.next().fri();
	...
	/** 
	 * Moves the date to the previous instance of a date as specified by the subsequent date element function (eg. .day(), .month()), month name function (eg. .january(), .jan()) or day name function (eg. .Sexta(), fri()).
	 * Example
	<pre><code>
	Date.today().last().Sexta();
	Date.today().last().fri();
	...
	/** 
	 * Creates a new Date (Date.today()) and moves the date to the previous instance of the date as specified by the subsequent date element function (eg. .day(), .month()), month name function (eg. .january(), .jan()) or day name function (eg. .Sexta(), fri()).
	 * Example
	<pre><code>
	Date.last().Sexta();
	Date.last().fri();
	 ...
	 * Example
	<pre><code>
	Date.today().is().Sexta(); // true|false
	Date.today().is().fri();
	Date.today().is().march();
	Date.today().is().mar();
	</code></pre>
	...
	var dx = ("Domingo Segunda Terça Quarta Quinta Sexta Sabado").split(/\s/),
   ...
   /** 
	...
	// Create day name functions and abbreviated day name functions (eg. Segunda(), Sexta(), fri()).
	...
	var df = function (n) {
		return function () {
			...
			if (this._nth !== null) {
				...
				//   Date.march().second().Segunda();
				// 
				// Things get crazy with the following...
				//   Date.march().add(1).second().second().Segunda(); // but it works!!
				//  
				if (this._isSecond) {
...
(function () {
	...
	var utils = {
		...
		getDayOfYearFromWeek : function (obj) {
			...
			jan4 = d.getDay() === 0 ? 7 : d.getDay(); // JS is 0 indexed on Domingo.
			offset = jan4+3;
	...
	$P.Normalizer = {
		...
		keys : function(){
			return [
				...
				utils.getDateNthString(false, true, "Segunda"),		//last mon
				utils.getDateNthString(false, true, "Terça"),		//last tues
				utils.getDateNthString(false, true, "Quarta"),	//last wed
				utils.getDateNthString(false, true, "Quinta"),	//last thurs
				utils.getDateNthString(false, true, "Sexta"),		//last fri
				utils.getDateNthString(false, true, "Sabado"),	//last sat
				utils.getDateNthString(false, true, "Domingo")		//last sun
			];
		},
...
(function () {
	...
	// Returns the date of the next Quinta.
	var d1 = Date.parse("Next Quinta");

	// Returns the date of the most previous Segunda.
	var d1 = Date.parse("last Segunda");
...
(function () {
	 ...
	 * %A		full weekday name according to the current localed							"Domingo" through "Sabado"
	 ...
	 * %u		weekday as a decimal number ["1", "7"], with "1" representing Segunda		"1" to "7"
	 * %U		week number of the current year as a decimal number, starting with the		"0" to ("52" or "53")
	 *		first Domingo as the first day of the first week
	 * %V		The ISO 8601:1988 week number of the current year as a decimal number,		"00" to ("52" or "53")
	 *		range 01 to 53, where week 1 is the first week that has at least 4 days
	 *		in the current year, and with Segunda as the first day of the week.
	 *		(Use %G or %g for the year component that corresponds to the week number
	 *		for the specified timestamp.)
	 * %W		week number of the current year as a decimal number, starting with the		"00" to ("52" or "53")
	 *		first Segunda as the first day of the first week
	 * %w		day of the week as a decimal, Domingo being "0"								"0" to "6"
	 ...
	 * l		A full textual representation of the day of the week (lowercase "L")		"Domingo" through "Sabado"
	 * N		ISO-8601 numeric representation of the day of the week (added in PHP 5.1.0)	"1" (for Segunda) through "7" (for Domingo)
	 * S		English ordinal suffix for the day of the month, 2 characters				"st", "nd", "rd" or "th". Works well with j
	 * w		Numeric representation of the day of the week								"0" (for Domingo) through "6" (for Sabado)
	 * z		The day of the year (starting from "0")										"0" through "365"
	 * W		ISO-8601 week number of year, weeks starting on Segunda						"00" to ("52" or "53")
